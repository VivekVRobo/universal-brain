...
def is_resource_authorized(pattern: str, target: str) -> bool:
    ...
    if clean_pattern.endswith("*"):
        ...
        # Sibling prefix without delimiter boundary (e.g. 'safe*') does NOT match 'safeevil'
        return False

    # Path relative_to check for local filesystem paths
    try:
...
def compute_canonical_request_digest(
    ...
    """Computes deterministic SHA-256 over canonical tool execution request."""
    canonical_payload = {
...
class CapabilityToken(BaseModel):
    ...
    def is_expired(self, current_time: Optional[datetime] = None) -> bool:
        """Check if token has passed its expiration deadline."""
...
class CapabilityService:
    ...
    def verify_token(
        ...
        """
        Validates token signature, expiration, target resource, request digest, and contract version (ALN-015).
        Raises CapabilityDeniedError if any condition fails.
        """
        ...
        # 3. Verify contract version freshness (ALN-015)
        ...
        # 4. Verify request digest if token is request-bound
        ...
        # 5. Verify target resource scope with boundary-safe matching
